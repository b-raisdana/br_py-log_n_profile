...
def get_changelog_body(repo_root: Path) -> str:
    result = subprocess.run(["git", "describe", "--tags", "--abbrev=0"], cwd=repo_root, capture_output=True, text=True)
...
def main():
    ...
    if current_branch != "main":
        print(f"ERROR: Must be on 'main' to release. Current branch: {current_branch}")
        sys.exit(1)
    ...
    run(
        [sys.executable, "-m", "twine", "upload", "dist/*", "-u", "__token__", "-p", pypi_api_key],
        cwd=repo_root,
    )

    run(["git", "add", "pyproject.toml", "CHANGELOG.md"], cwd=repo_root)
    ...
    print(f"View at: https://pypi.org/project/br-logging-and-profiling/{new_version}/")
